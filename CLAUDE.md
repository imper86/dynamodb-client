...
`getResourcePolicy`, `importTable`, `listBackups`, `listContributorInsights`, `listExports`, `listImports`, `listTables`, `listTagsOfResource`, `putItem`, `putResourcePolicy`, `query`, `restoreTableFromBackup`, `restoreTableToPointInTime` — are the templates.
