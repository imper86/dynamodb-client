...
`getResourcePolicy`, `importTable`, `listBackups`, `listContributorInsights`, `listExports`, `listImports`, `listTables`, `listTagsOfResource`, `putItem`, `putResourcePolicy`, `query`, `restoreTableFromBackup`, `restoreTableToPointInTime`, `scan`, `searchVectors`, `tagResource`, `transactGetItems`, `transactWriteItems`, `untagResource` — are the templates.
